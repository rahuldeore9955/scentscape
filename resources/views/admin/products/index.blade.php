...
@section('content')
    <div class="panel-welcome"><span class="panel-eyebrow">Catalog</span><h1>Products</h1><p>Create products and keep pricing and storefront visibility current.</p></div>
    <section class="panel-card">
        ...
        <div class="panel-table-wrap"><table class="panel-table"><thead><tr><th>Product</th><th>Product ID</th><th>Price</th><th>Status</th><th>Actions</th></tr></thead><tbody>@forelse($products as $product)<tr><td><strong>{{ $product->name }}</strong></td><td>{{ $product->sku }}</td><td>Rs. {{ number_format((float) $product->price, 2) }}</td><td><span class="admin-status {{ str_replace('_', '-', $product->status) }}">{{ ucfirst(str_replace('_', ' ', $product->status)) }}</span></td><td><div class="admin-actions"><a class="panel-link-btn" href="{{ route('admin.products.edit', $product) }}">Edit</a><form method="POST" action="{{ route('admin.products.destroy', $product) }}" onsubmit="return confirm('Remove this product from the catalog?')">@csrf @method('DELETE')<button class="panel-delete-btn" type="submit">Remove</button></form></div></td></tr>@empty<tr><td colspan="5" class="panel-empty">No products are available yet.</td></tr>@endforelse</tbody></table></div>
    </section>
@endsection
