...
@section('content')
    ...
    <section class="panel-card"><form method="POST" action="{{ $editing ? route('admin.products.update', $product) : route('admin.products.store') }}" class="panel-form" enctype="multipart/form-data">@csrf @if($editing) @method('PUT') @endif
        <h2 class="panel-form-section-title">Product Information</h2><div class="panel-form-grid">
            ...
            <label>Slug<input value="{{ $editing ? $product->slug : 'Generated from product title' }}" readonly></label>
            <label>Product ID<input value="{{ $editing ? $product->sku : 'Generated automatically' }}" readonly></label>
            @if($editing)
                <label>Category<input value="{{ ucfirst($product->category) }}" readonly><input type="hidden" name="category" value="{{ $product->category }}"></label>
            @else
                <label>Category<select name="category">@foreach(['women' => 'Women', 'men' => 'Men', 'unisex' => 'Unisex'] as $value => $label)<option value="{{ $value }}" @selected(old('category', 'unisex') === $value)>{{ $label }}</option>@endforeach</select></label>
            @endif
            <label>Status<select name="status">@foreach(['active' => 'Active', 'inactive' => 'Inactive'] as $value => $label)<option value="{{ $value }}" @selected(old('status', $product->status ?: 'active') === $value)>{{ $label }}</option>@endforeach</select></label>
            <label>Price (Rs.)<input type="number" name="price" min="0" step="0.01" value="{{ old('price', $product->price) }}" required></label>
            <label>Badge<input name="badge" value="{{ old('badge', $product->badge) }}" placeholder="bestseller, new, or sale"></label>
            <label>Size<input name="size" value="{{ old('size', $product->size) }}" placeholder="e.g. 100 ml"></label>
        </div>
        <div class="admin-product-images-grid">
            ...
            <div class="admin-product-image-field">
                <label><span class="admin-image-field-heading">Extra Images <span class="admin-field-hint">Up to 4 gallery images</span></span><input type="file" name="images_upload[]" accept="image/jpeg,image/png,image/webp" multiple></label>
